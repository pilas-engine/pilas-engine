class Fisica {
  ...
  realizar_rayo_entre(desde_x: number, desde_y: number, hasta_x: number, hasta_y: number, figuras = undefined) {
    let desde = this.pilas.utilidades.convertir_coordenada_de_pilas_a_phaser(desde_x, desde_y);
    let hasta = this.pilas.utilidades.convertir_coordenada_de_pilas_a_phaser(hasta_x, hasta_y);

    figuras = figuras || this.figuras;

    let data = this.Matter.Query.ray(figuras, desde, hasta);
