...
module('Integration | Component | pilas propiedad', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    await render(hbs`{{pilas-propiedad/numero objeto=objeto propiedad=propiedad}}`);
    assert.ok(find('*').textContent.trim(), 'tiene que tener texto');
    assert.ok(find('*').textContent.trim().indexOf('100') > -1, 'debe aparecer el valor de la propiedad');
    assert.ok(find('*').textContent.trim().indexOf('rotacion') > -1, 'debe aparecer la etiqeuta de la propiedad');
  });
});
