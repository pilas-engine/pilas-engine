...
test("it renders", function(assert) {
  ...
  this.set("cuandoInicia", pilas => {
    ...
    assert.equal(pilas.utilides.convertir_radianes_a_angulos(0), 0);
    assert.equal(pilas.utilides.convertir_radianes_a_angulos(Math.PI), 180);
    assert.equal(pilas.utilides.convertir_angulo_a_radianes(90), Math.PI / 2);
    assert.equal(pilas.utilides.convertir_angulo_a_radianes(180), Math.PI);
  });
